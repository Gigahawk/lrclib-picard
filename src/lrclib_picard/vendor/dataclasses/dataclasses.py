...
# Picard is packaged with PyInstaller which seems to not include
# dataclasses, package it ourself
# Copied from https://github.com/python/cpython/blob/v3.8.10/Lib/dataclasses.py
    ...
    if order:
        ...
        for name, op in [
            ...
            if _set_new_attribute(
                cls, name, _cmp_fn(name, op, self_tuple, other_tuple, globals=globals)
            ):
                raise TypeError(
...
def replace(*args, **changes):
    ...
    if args:
        ...
        warnings.warn(
            "Passing 'obj' as keyword argument is deprecated",
            DeprecationWarning,
            stacklevel=2,
        )
    else:
